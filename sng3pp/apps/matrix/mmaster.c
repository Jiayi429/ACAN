...
{ 	struct sigaction sa;
	...
	fclose(fd);
	sprintf(query,"insert into ScalabilityLog (Timestamp,LogID,Size,ElapsedTime,MFLOPS,ChunkSize,NumofHosts) values (\"%s\",\"%s\",%d,%f,%f,%d,0)",
			timeStamp,host,N*1,t1/1000000,(float) N*N/t1*N,G);
	if (logDB) {
